...
# Read the Excel file
df = pd.read_excel('/Users/hothaifa/Desktop/s3/ארתמטיקה כתה ד 500 שאלות.xlsx')

# Create SQL file for INSERT commands
with open('insert_data3.sql', 'w') as insert_data_file:
    ...
    for index, row in df.iterrows():
        ...
        question_text = row.iloc[2].replace("'", "`") if isinstance(row.iloc[2],str) else row.iloc[2]  # Assuming the third column (index 2) is qoustion_text
        explanation = ""  # You can add explanation if available in your dataset
        interesting_fact = row.iloc[7].replace("'", "`") if isinstance(row.iloc[7],str) else row.iloc[7]  # Assuming the eighth column (index 7) is intersting fact
        print(row.iloc[7])
